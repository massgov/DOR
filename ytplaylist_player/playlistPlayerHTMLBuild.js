...
function pageHeightWidth() {
	...
	console.log('x = ' + x + ': y = ' + y + " g: " + g + " e:" + e);

	playgroundHeight = y;
	vidPlayerWidth = x;
	console.log("height = " + playgroundHeight + " width = " + vidPlayerWidth);
}

// calcuates the best layout
function playerPresentation() {
	...
	if (vidPlayerHeight > vidPlayerWidth) {
		...
		vidPagePortrait = false;
	}

	if (vidPlayerHeight < mobileLandscapeBreak) {
...
function buildHTMLPlayerDivs() {
	...
	for (var i = 0; i < ytPlaylists.length; i++) {
		...
		console.log("initial vidPlayerHeight this is " + vidPlayerHeight);
